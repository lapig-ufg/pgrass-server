node {
    ...
    def NODE_VERSION= "v14.17.3"
		...
		def scannerHome = tool 'sonarqube-scanner';
                    ...
                    -Dsonar.projectKey=pgrass-server \
                    -Dsonar.sources=. \
                    -Dsonar.css.node=. \
        ...
        stage('Build') {
                        ...
                        def status = sh(returnStatus: true, script: "cd client && ng build -c production  --aot=true --no-progress")
                        if (status != 0) {
                            echo "FAILED BUILD!"
        ...
        stage('Deploy container on PROD') {
                        ...
                        configFileProvider([configFile(fileId: "$File_Json_Id_APP_LAPIG_PGRASS_SERVER_PROD", targetLocation: 'container-lapig-pgrass-server-deploy-prod.json')]) {
